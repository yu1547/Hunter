...
object MissionHandler {
     ...
     * @param supplyId 補給站的 ID
     * @return CheckSpotMissionResponse 回傳操作結果，包含訊息和任務是否完成的狀態
     * @throws Exception 如果 API 請求失敗
     */
    suspend fun checkSpotMission(userId: String, supplyId: String): CheckSpotMissionResponse {
        return try {
            val response = apiService.checkSpotMission(userId, supplyId)
